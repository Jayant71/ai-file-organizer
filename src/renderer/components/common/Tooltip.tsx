/**
 * Tooltip Component
 * Provides helpful hints and explanations for non-technical users.
 */
        ...
        >
            ...
            {isVisible && (
                <div
                    ...
                    role="tooltip"
                >
                    <div className="px-3 py-2 text-sm text-white bg-slate-800 dark:bg-slate-600 rounded-lg shadow-lg max-w-xs">
                        {content}
                    </div>
                    <div
...
export function HelpIcon({ tooltip }: { tooltip: string | React.ReactNode }) {
    return (
        <Tooltip content={tooltip} position="top">
            <button
                type="button"
                className="inline-flex items-center justify-center w-4 h-4 ml-1 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 bg-slate-200 dark:bg-slate-700 rounded-full cursor-help transition-colors"
                aria-label="Help"
            >
                ?
            </button>
